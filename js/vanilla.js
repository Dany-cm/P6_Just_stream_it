...
const onImgClicked = function (event) {
    ...
    fetch(`${api_url}${movie_id}`)
        ...
        .then(movie => {
            ...
            document.getElementsByClassName("movie-titles")[0].innerHTML = `<p><strong>Titre: </strong>${movie.title}</span>`
            document.getElementsByClassName("movie-genre")[0].innerHTML = `<p><strong>Genre(s): </strong>${movie.genres}</span>`
            document.getElementsByClassName("movie-release-date")[0].innerHTML = `<p><strong>Date de sortie: </strong>${movie.date_published}</span>`
            document.getElementsByClassName("movie-rated")[0].innerHTML = `<p><strong>Rated: </strong>${movie.rated}</span>`
            document.getElementsByClassName("movie-rating-imdb")[0].innerHTML = `<p><strong>Score IMDB: </strong>${movie.imdb_score}</span>`
            document.getElementsByClassName("movie-director")[0].innerHTML = `<p><strong>Réalisateur(s): </strong>${movie.directors}</span>`
            document.getElementsByClassName("movie-list-actors")[0].innerHTML = `<p><strong>Liste des acteurs: </strong>${movie.actors}</span>`
            document.getElementsByClassName("movie-duration")[0].innerHTML = `<p><strong>Duration: </strong>${movie.duration} minutes </span>`
            document.getElementsByClassName("movie-country-origin")[0].innerHTML = `<p><strong>Origine(s): </strong>${movie.countries}</span>`
            document.getElementsByClassName("movie-box-office-result")[0].innerHTML = `<p><strong>Résultat du box office: </strong>${movie.worldwide_gross_income ?? 'Non disponible'}</span>`
            document.getElementsByClassName("movie-description")[0].innerHTML = `<p><strong>Synopsis: </strong>${movie.description}</span>`
